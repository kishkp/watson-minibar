 ...
 * classifier.name must match 'minibarAnalyzer'.
    ...
    this.vizRecClient.listClassifiers({}, (err,response) => {
      if (err) {
        ...
        for (let i = 0, size = classifiers.length; i < size; i++) {
          ...
          if (classifier.name === 'minibarAnalyzer') {
 ...
 * ('minibarAnalyzer') so that we can find it later.
...
WatsonVisRecSetup.prototype.createVisRecClassifier = function(params) {
  ...
  return new Promise((resolve, reject) => {
    ...
    var createClassifierParams = {
        name: 'minibarAnalyzer',
        FullMinibar_positive_examples: fs.createReadStream('./data/FullMinibar.zip'),
        MinibarWithoutCans_positive_examples: fs.createReadStream('./data/MinibarWithoutCans.zip'), 
        MinibarWithoutChocolates_positive_examples: fs.createReadStream('./data/MinibarWithoutChocolates.zip')
      }
